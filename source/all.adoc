...
BUFR is Binary Universal Form for (mostly meteorological) data Representation. The current edition is 4. BUFR is a continous bit stream(Section 4) with header(Section1) and contents(Section3). The data is coded by tables. The tables are defined by WMO, the current tables version is 34. There are 3 table types

* Element table(B) : define data name, unit, range/precision
* Sequence table(D) : define sequence of elements/sequences
* Code/Flag table(C ) : define codes, categories

The E-SOH system must map table elements to CF variables.

BUFR as input:

The most of BUFR messages are edition 4, but there are few older (edition 3) too. There are many different implementations of BUFR encoding. A lot of manufacturers shipped their own solution with new devices. These devices are out of supported/warrantly period, no hope to harmonize incoming BUFR messages. The tables defined by WMO, but anybody can create a new tables. Sometimes it causes conflict between global(WMO) tables and local(user defined) tables. There are not any error-detecting code(for example CRC) in the BUFR message. The E-SOH system discards the incoming message when the size(Section0) and the end(Section5) are not match.

BUFR as output:

WMO defined BUFR sequence templates to represent the observations. This is recomended to use for E-SOH system output generation. WIGOS identifiers can be included in some BUFR templates:
