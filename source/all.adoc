...
In the scope of E-SOH, we define a dataset as the combination of one instrument on one observation platform, with the associated measurements as given by their standard names according to the link:https://cfconventions.org/Data/cf-standard-names/current/build/cf-standard-name-table.html[CF convention standard names vocabulary].

* Time coverage: Each dataset have a validity time of maximum 24 hours. Until its validity time has become 24 hours, the dataset will grow due to addition of new data.
* Each dataset is dynamically created and deleted. This is a consequence of the rolling buffer design of the E-SOH data store.
* If a platform does not create a reading for a certain parameter during 24 hours, there will be no dataset for this period. If such an observation where to be submitted again at a later time, a new dataset will be created.
